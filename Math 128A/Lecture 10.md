...
$$
so what we do is to choose a positive integer $N$, and select **mesh points** $$
...
- then recursively $$
\begin{aligned}
\underline{|y(t_{j+1})-w_{j+1}|+\frac{hM}{2L}}\leq\quad(1+hL)\left|y(t_j)-w_j\right|+\frac{h^2}{2}M+\frac{hM}{2L} \\
